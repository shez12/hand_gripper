...
class AnyDevice(gatt.Device):
    ...
    def __init__(self, manager, mac_address):
        ...
        rospy.init_node('imu_quaternion_publisher', anonymous=True)
        self.quat_pub = rospy.Publisher('imu_quaternion', Quaternion, queue_size=10)
    ...
    def parse_imu(self,buf):
        ...
        if buf[0] == 0x11:
            ...
            if ((ctl & 0x0020) != 0):
                ...
                quat_msg = Quaternion()
                quat_msg.w = float(tmpAbs)
                quat_msg.x = float(tmpX)
                quat_msg.y = float(tmpY)
                quat_msg.z = float(tmpZ)
