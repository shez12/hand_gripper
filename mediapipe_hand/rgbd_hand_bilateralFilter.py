...
try:
    ...
    while True:
        ...
        current_frame=depth_image.astype(np.float32)

        if prev_frame is not None:
            frame_diff=depth_image-prev_frame
            smoothed_diff=cv2.bilateralFilter(frame_diff,9,75,75)

            smoothed_frame=np.add(prev_frame,smoothed_diff)
            depth_image=smoothed_frame
