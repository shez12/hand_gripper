...
import time
import datetime
from gesture import detect_all_finger_state, number_gesture,calculate_distance
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for handedness, hand_landmarks in zip(results.multi_handedness, results.multi_hand_landmarks):
                ...
                bend_states, straighten_states=detect_all_finger_state(data_pixel)
                
                if start_video==0 and number_gesture(bend_states, straighten_states)=='5':
                    start_video=1
                if start_video==1 and number_gesture(bend_states, straighten_states)=='0':
                    start_video=2
                    print('strat record video')
                
                if start_video==2 and start_record_move==0 and number_gesture(bend_states, straighten_states)=='1':
                    start_record_move=1
                    print('...')
                    last_gesture_time = time.time()  # 记录第一次检测到手势“1”的时间
 
                if start_video==2 and start_record_move==1 and time.time() - last_gesture_time >= wait_time :
                    start_record_move=2
                    print('frame '+str(len(data_point_cloud_save))+' strat move')

                if start_record_move==2 and end_record_move==0 and number_gesture(bend_states, straighten_states)=='1':
                    end_record_move=2
                    print('frame '+str(len(data_point_cloud_save))+' end move')

                if start_video==2 and end_video==0 and number_gesture(bend_states, straighten_states)=='2':
                    end_video=1
                if start_video==2 and end_video==1 and number_gesture(bend_states, straighten_states)=='0':
                    end_video=2
        ...
        if start_video==2:
            ...
            #保存正则化的点云数据
            data_point_cloud_save.append(np.array(data_point_cloud).flatten())
        if end_video==2:
            #点云写入csv
            df = pd.DataFrame(data_point_cloud_save, columns= [f'{i}_{c}' for i in range(21) for c in ['x_norm', 'y_norm', 'z_norm']])
            df.to_csv(norm_point_csv_save_path, index=False)
            break
